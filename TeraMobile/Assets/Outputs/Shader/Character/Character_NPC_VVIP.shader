Shader "TERA/Character/NPC_VVIP" {
    Properties {
		...
		shiftmap("Tangent Shift Map", 2D) = "black" {}
    }
    SubShader {
        Tags {
...
///// SkinColor:
                ...
                float3 CCparam = RGBtoHSV(haircolor);
...
///// Lightness:
                ...
                float3 IBL = ImageBasedLightingNPC(gloss,viewReflectDirection);
//                       IBL *= (1-hairrange) * sh;
                float3 zis = Frsn(Ndotv,gloss,IBL,equiprange);
                float3 _Rim = Rim(NdotV,_RimPower,_RimColor);

////// Specular:
                float spGGX = SpecularGGXWithHeadLight(pie, gloss, _MatMask_var.r, max(Ndothd,Ndoth), Ndotv, NdotV, NdotL,_NdotL)*(1-hairrange);
                ...
                float sPc_hair = SpecAniso(_AnisoDir, viewDirection, normalDirection, _AnisoOffset2, _Glossiness, 128, aspc1+CCsatfix);
                      sPc_hair += SpecAniso(_AnisoDir, viewDirection, normalDirection, _AnisoOffset, _Glossiness2, 128, aspc2);
                      sPc_hair += dsmask*2*(1-sPc_hair+CCsatfix);
                      sPc_hair *= hairrange;
                float3 specular = CalculateSpecular(_SpecularmapRGBA_var.rgb,spGGX,gloss,attenColor,IBL*(1-hairrange)*sh,equiprange,sPc_skin,max(0.04,sPc_hair));
...
/////// Diffuse:
                ...
                float3 diffuse = CalculateDiffuse(NdotLs,NdotLq,scattering,attenColor,sh,baseRGBA);
                       diffuse = lerp(diffuse,_skincolor*(1+_SkinDirpow*NdotL),NdotL*_MatMask_var.r)*(1-hairrange);
                       diffuse += CalculateHairDiffuseNPC(NdotL,Ndotl,_NdotL,Ndotv,attenColor,haircolor,hairrange,CCparam.y,CCvalue);
                       diffuse += Scattering(Ndotv, 20, 0.05,hairrange) * sh * 0.5;
                       diffuse += sssadd;
...
//                       finalColor = CalculateHairDiffuseNPC(NdotL,Ndotl,_NdotL,Ndotv,attenColor,haircolor,hairrange,CCparam.y,CCvalue);
//                       finalColor = CalculateHairDiffuse(NdotL,Ndotl,_NdotL,Ndotv,fixed3(1,1,1),attenColor,haircolor,hairrange,CCparam.y,CCvalue);

//                       finalColor = _MatMask_var.rgb;//sPc_hair;//dsmask*2*(1-sPc_hair+CCsatfix);
...
///// Lightness:
                ...
                float3 _Rim = Rim(NdotV,_RimPower,_RimColor);
////// Specular:
                float spGGX = SpecularGGX(3.1415926535, gloss, _MatMask_var.r, max(Ndothd,Ndoth), NdotV, NdotL);
                float3 sPc_skin = Specularskin(gloss, max(Ndothd,Ndoth), _lf, spGGX, NdotL, attenColor, Ndotv, _headlight, Ndotl, _skinspcolor, specmncrm);
...
/// Final Color:
    ...
    SubShader {
        ...
        Pass {
            ...
            half4 frag4(V2f_TeraDiffuse i) : COLOR {
                i.normalDir = normalize(i.normalDir);
                float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
