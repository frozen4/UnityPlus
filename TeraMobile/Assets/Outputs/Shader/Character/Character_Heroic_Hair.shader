...
//        _tc ("tc", 2D) = "bump" {}
        ...
        [MaterialToggle] _iblchk("IBL check",Range(0,1)) = 0
        [MaterialToggle] _difchk("Diffuse check",float) = 0
        [MaterialToggle] _ndachk("aniso check",Range(0,1)) = 0
        [MaterialToggle] _colchk("color check",float) = 0
    }
    SubShader {
        Tags {
...
//                float NdotA = max(0,dot(i.normalDir+tangentshift,ahalfvec));
                ...
                float3 sh = AmbientColorGradient(normalDirection);// * (pow(NdotL,0.7)*0.5+0.5);
...
/// Final Color:
                float3 finalColor = diffuse + specular + _Rim;
                       ...
                       finalColor = lerp(finalColor,specular,_spchk);
                       finalColor = lerp(finalColor,IBL,_iblchk);
                       finalColor = lerp(finalColor,diffuse,_difchk);
                       finalColor = lerp(finalColor,NdotA,_ndachk);
