Shader "Character/Character_Heroic_Face3" {
        Properties {
...
//                float3 NdotLs = LightingbyBRDFmap(_Brdfmap, NdotO, VdotO, max(0,1-NdotO), _brdfrange, 0.5, _headlight, 0.1);
                float3 NdotLs = LightingbyBRDFmap2(_Brdfmap, NdotL, Ndotv, Ndotl, _brdfrange, 0.5, _headlight, 0.1);
//                       NdotLs *= lerp(NdotLs,1,NdotO+(VdotO*max(0,1-NdotO)));
//                       NdotLs = saturate(NdotL + _brdfrange + (Ndotv*_headlight*_NdotL));
                ...
                float3 diffuse = CalculateDiffuse(NdotLs,0,scattering,attenColor,sh,_skincolor) * lerp(i.pl,1,_skinpoint);
                       diffuse = lerp(diffuse,_skincolor*(1+_SkinDirpow*NdotL),NdotL);
                       diffuse += sssadd;
///// Eye Color:
...
//                       finalColor = NdotL * _skincolor;
//                       finalColor = sh*_skincolor*1.5;
