Shader "Tera/MobileDiffuseBumpwithRimEffect"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f_mdbd i) : SV_Target
			{
			    ...
			    float3 normalDir = UnpackNormal(tex2D(_BumpMap, i.uv.zw));
	            TangentToWorld(i.tangentToWorld, normalDir);
	            float NdotV = saturate(1 - dot(normalDir, viewDir));
				...
				half atten = 1;
				half3 _Rim = pow(NdotV,_RimPower) * _RimColor.rgb * max(0,_RimPower);				
