...
namespace Colorful
{
	...
	public class ShadowsMidtonesHighlights : BaseEffect
	{
        ...
        private int _ShadowsId = 0;
        private int _MidtonesId = 0;
        private int _HighlightsId = 0;
        private int _AmountId = 0;

        void Awake()
        {
            _ShadowsId = Shader.PropertyToID("_Shadows");
            _MidtonesId = Shader.PropertyToID("_Midtones");
            _HighlightsId = Shader.PropertyToID("_Highlights");
		...
		protected override void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			float multiplier;
            Material.SetVector(_ShadowsId, Shadows * (Shadows.a * 2));
			multiplier = 1f + (1f - (Midtones.r * 0.299f + Midtones.g * 0.587f + Midtones.b * 0.114f));
            Material.SetVector(_MidtonesId, (Midtones * multiplier) * (Midtones.a * 2f));
			multiplier = 1f + (1f - (Highlights.r * 0.299f + Highlights.g * 0.587f + Highlights.b * 0.114f));
            Material.SetVector(_HighlightsId, (Highlights * multiplier) * (Highlights.a * 2f));
