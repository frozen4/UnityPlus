...
namespace gcloud_voice
{
	...
	{
        static void DevLog(object message)
        {
           // Debug.Log(message);
        }
...
#region DllImport
		...
		public GCloudVoiceEngine()
		{
			...
			if(ret != 0)
			{
				DevLog("Create GCloudVoiceInstance failed!");
			}
		...
		public override int Init()
		{
			if(!bIsSetAppInfo)
			{
				DevLog("please set appinfo first");
			...
			if(!bInit)
			{
				...
				if(ret != 0)
				{
					DevLog("Init GCloudVoice failed!");
					return ret;
				}
				bInit = true;
			}
		...
		{
			DevLog("GCloudVoice_C# API: _SetMode");
			if (!bInit)
			{
		...
		public NoticeMessage NoticeMessage_ParseFrom(byte[] buf, int buflen)
		{
			...
			if (buflen - guard < sizeof(Int32)) {
				DevLog("notifymsg,parse error, buf small then sizeof(int)");
				return null;
			}
		...
		public  override  int Poll()
		{
			...
			if (msg.what == (int)NoticeMessageType.MSG_ON_JOINROOM_COMPLETE) {
				if (OnJoinRoomComplete != null) {
					DevLog ("c# poll callback OnJoinRoomComplete not null");
            ...
            else if (msg.what == (int)NoticeMessageType.MSG_ON_ROLE_CHANGED)
            {
                ...
                {
                    //DevLog("ChangeRole array num : [" + msg.custom[0] + "," + msg.custom[1] + "," + msg.custom[2] + "," + msg.custom[3]);
                    OnRoleChangeComplete((IGCloudVoice.GCloudVoiceCompleteCode)msg.intArg1, msg.strArg, msg.intArg2, msg.custom[0]);
                }
            }
            else if (msg.what == (int)NoticeMessageType.MSG_ON_EVENT_NOTIFY)
            {
                if (OnEvent != null)
                {
                    OnEvent((GCloudVoiceEvent)msg.intArg1, msg.strArg);
						...
						else if(msg.what == (int)NoticeMessageType.MSG_ON_MUTE_STATE)
						{
		...
		{
			DevLog("GCloudVoice_C# API: _Pause");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: _Pause nRet=" + nRet);
			return nRet;
		}

		public  override int Resume()
		{
			DevLog("GCloudVoice_C# API: _Resume");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: _Resume nRet=" + nRet);
			return nRet;
		}

		public override int JoinTeamRoom(string roomName, int msTimeout)
		{
			DevLog("GCloudVoice_C# API: JoinTeamRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: JoinTeamRoom  nRet=" + nRet);
			return nRet;
		}

		public override int JoinRangeRoom(string roomName, int msTimeout)
		{
			DevLog("GCloudVoice_C# API: JoinRangeRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: JoinRangeRoom  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: JoinTeamRoom"+" mstimeout:"+msTimeout);
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
		...
		{
			DevLog("GCloudVoice_C# API: JoinNationalRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: JoinNationalRoom  nRet=" + nRet);
			return nRet;
		}

        public override int ChangeRole(GCloudVoiceRole role, string roomName="")
        {
            DevLog("GCloudVoice_C# API: ChangeRole");
            if (!bInit)
            {
                return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
            }
            int nRet = GCloudVoice_ChangeRole((int)role, roomName);
            DevLog("GCloudVoice_C# API: GCloudVoice_ChangeRole  nRet=" + nRet);
            return nRet;
        }

		public override int JoinFMRoom(string roomID,int msTimeout)
		{
			DevLog("GCloudVoice_C# API: JoinFMID");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			//DevLog("GCloudVoice_C# API: JoinFMRoom  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: QuitRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: QuitRoom  nRet=" + nRet);
			return nRet;
		}

		public override int OpenMic()
		{
			DevLog("GCloudVoice_C# API: OpenMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: OpenMic  nRet=" + nRet);
			return nRet;
		}

		public override int CloseMic()
		{
			DevLog("GCloudVoice_C# API: CloseMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: CloseMic  nRet=" + nRet);
			return nRet;
		}

		public override int OpenSpeaker()
		{
			DevLog("GCloudVoice_C# API: OpenSpeaker");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: OpenSpeaker  nRet=" + nRet);
			return nRet;
		}

		public override int CloseSpeaker()
		{
			DevLog("GCloudVoice_C# API: CloseSpeaker");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: CloseSpeaker  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: EnableRoomMicrophone");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: EnableRoomMicrophone  nRet=" + nRet);
			return nRet;
		}

		public override int EnableRoomSpeaker(string roomName, bool enable)
		{
			DevLog("GCloudVoice_C# API: EnableRoomSpeaker");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: EnableRoomSpeaker  nRet=" + nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: EnableMultiRoom");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: EnableMultiRoom  nRet=" + nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: UpdateCoordinate");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: UpdateCoordinate  nRet=" + nRet);
			return nRet;
		}

		public override int ApplyMessageKey(int msTimeout)
		{
			DevLog("GCloudVoice_C# API: ApplyMessageKey");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: ApplyMessageKey  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: SetMaxMessageLength");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: SetMaxMessageLength  nRet=" + nRet);
			return nRet;
		}

		public override int StartRecording(string filePath)
		{
			DevLog("GCloudVoice_C# API: StartRecording");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: StartRecording  nRet=" + nRet);
			return nRet;
		}

		public override int StopRecording ()
		{
			DevLog("GCloudVoice_C# API: StopRecording");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: StopRecording  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: UploadRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: UploadRecordedFile  nRet=" + nRet);
			return nRet;
		}

		public override int DownloadRecordedFile(string fileID, string downloadFilePath, int msTimeout)
		{
			DevLog("GCloudVoice_C# API: DownloadRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: DownloadRecordedFile  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: PlayRecordedFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: PlayRecordedFile  nRet=" + nRet);
			return nRet;
		}

		public override int StopPlayFile()
		{
			DevLog("GCloudVoice_C# API: StopPlayFile");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: StopPlayFile  nRet=" + nRet);
			return nRet;
		}

		public override int SpeechToText(string fileID, int language = 0, int msTimeout = 6000)
		{
			DevLog("GCloudVoice_C# API: SpeechToText");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: SpeechToText  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: ForbidMemberVoice");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: ForbidMemberVoice  nRet=" + nRet);
			return nRet;
		}

		public override int EnableLog(bool enable)
		{
			DevLog("GCloudVoice_C# API: EnableLog");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: EnableLog  nRet=" + nRet);
			return nRet;
		}

		public override int SetDataFree(bool enable)
		{
			DevLog("GCloudVoice_C# API: SetDataFree");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: SetDataFree  nRet=" + nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: GetMicLevel");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: GetMicLevel  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: GetSpeakerLevel");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: GetSpeakerLevel  nRet=" + nRet);
			return nRet;
		}

		public override int SetSpeakerVolume(int vol)
		{
			DevLog("GCloudVoice_C# API: SetSpeakerVolume");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: SetSpeakerVolume  nRet=" + nRet);
			return nRet;
		}

		public override int TestMic()
		{
			DevLog("GCloudVoice_C# API: TestMic");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: TestMic  nRet=" + nRet);
			return nRet;
		}

		public override int GetFileParam(string filepath, int [] bytes, float [] seconds)
		{
			DevLog("GCloudVoice_C# API: GetFileParam");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: GetFileParam  nRet=" + nRet);
			return nRet;
		}

		public override int invoke( uint nCmd, uint nParam1, uint nParam2, uint [] pOutput )
		{
			DevLog("GCloudVoice_C# API: invoke");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: invoke  nRet=" + nRet);
			return nRet;
		}
		public override int EnableSpeakerOn(bool bEnable)
		{
			DevLog("GCloudVoice_C# API: EnableSpeakerOn");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_EnableSpeakerOn  nRet=" + nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: SetMicVol");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_SetMicVol  nRet=" + nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: SetVoiceEffects");
			if (!bInit)
			{
				return (int)GCloudVoiceErr.GCLOUD_VOICE_NEED_INIT;
			}
			...
			DevLog("GCloudVoice_C# API: SetVoiceEffects  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: EnableReverb");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_EnableReverb  nRet=" + nRet);
			return nRet;
		}
		...
		{
			DevLog("GCloudVoice_C# API: SetReverbMode");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_SetReverbMode  nRet=" + nRet);
			return nRet;

		}
		public override int GetVoiceIdentify()
		{
			DevLog("GCloudVoice_C# API: GetVoiceIdentify");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_GetVoiceIdentify nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: SetBGMPath");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_SetBGMPath nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: StartBGMPlay");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_StartBGMPlay nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: StopBGMPlay");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_StopBGMPlay nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: PauseBGMPlay");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_PauseBGMPlay nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: ResumeBGMPlay");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_ResumeBGMPlay nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: SetBGMVol");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_SetBGMVol nRet = "+ nRet);
			return nRet;
		...
		{
			//DevLog("GCloudVoice_C# API: GetBGMPlayState");
			...
			//DevLog("GCloudVoice_C# API: GCloudVoice_GetBGMPlayState nRet = "+ nRet);
			return nRet;
		...
		public override int EnableNativeBGMPlay(bool bEnable)
		{
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_EnableNativeBGMPlay nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: SetBitRate");
			...
			DevLog("GCloudVoice_C# API: GCloudVoice_SetBitRate nRet = "+ nRet);
			return nRet;
		...
		{
			DevLog("GCloudVoice_C# API: getAudioDeviceConnectionState");
			if (!bInit)
			{
				return (int)GCloudVoiceDeviceState.AUDIO_DEVICE_UNCONNECTED;
			}

			int nRet = GCloudVoice_GetAudioDeviceConnectionState();
			DevLog("GCloudVoice_C# API: getAudioDeviceConnectionState  nRet=" + nRet);
			return nRet;
		}

        public override void EnableBluetoothSCO(bool enable)
		{
			DevLog("GCloudVoice_C# API: EnableBluetoothSCO");
			if (!bInit)
			{
				return;
			}
        ...
        {
            DevLog("GCloudVoice_C# API: CheckDeviceMuteState");
            if (!bInit)
            {
                return (int)GCloudVoiceDeviceState.AUDIO_DEVICE_UNCONNECTED;
            }
