...
public class Gcloud_OffLineVoice : IVoiceModule
{
    private VoiceManager _Manager = null;
    private VoiceMode _Mode;
    private bool _bIsGetAuthKey = false;

    public Gcloud_OffLineVoice(VoiceManager manager, VoiceMode mode)
    {
        _Manager = manager;
        _Mode = mode;
    ...
    private void RecordingFileComplete(int code)
    {
        //call lua
        IntPtr L = LuaScriptMgr.Instance.GetL();
        if (L != IntPtr.Zero)
        {
            int oldTop = LuaDLL.lua_gettop(L);
            LuaDLL.lua_getglobal(L, "Voice_OnRecordingFileComplete");
            if (!LuaDLL.lua_isnil(L, -1))
            {
    ...
    public int UploadRecordedFile(int timeout)
    {
