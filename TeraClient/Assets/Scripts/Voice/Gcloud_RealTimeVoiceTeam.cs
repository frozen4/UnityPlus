...
public class Gcloud_RealTimeVoiceTeam : IVoiceModule
{
    ...
    public Gcloud_RealTimeVoiceTeam(VoiceManager manager, VoiceMode mode)
    {
        _Manager = manager;
        _Mode = mode;
    }
    ...
    public int JoinTeamRoom(string strTeamRoom, int timeout)
    {
        if (!_bRoomJoint)
        {
            return _Manager.Gcloud_VoiceMan.VoiceEngine.JoinTeamRoom(strTeamRoom, timeout);
        }
        return 0;
    }

    public int QuitTeamRoom(int timeout)
    {
        if (_bRoomJoint)
        {
