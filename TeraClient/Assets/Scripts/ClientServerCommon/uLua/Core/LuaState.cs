...
namespace LuaInterface
{
    ...
    public class LuaState : IDisposable
    {
        ...
        {            
            ...
            if (text == null)
            {                
                UnityEngine.Debug.LogError("Loader lua file failed: " + fileName);      
                LuaDLL.lua_pop(L, 1);
                return null;
            }
        ...
        //private readonly List<string> globals = new List<string>();     //瀹屽叏鏃犵敤鐨勯�楁瘮
...
//         }
...
#if true || UNITY_EDITOR	//在编辑器中输出Lua栈，有一定效率损失
        //如果不出错，则保留返回值在堆栈上
        //如果出错，压入调用堆栈信息，抛出异常
        public void Call(int nArgs)
        {
            ...
            /*
            int oldTop = LuaDLL.lua_gettop(L) - nArgs - 1;
            LuaDLL.lua_rawgeti(L, LuaIndexes.LUA_REGISTRYINDEX, errorFuncRef);
            LuaDLL.lua_insert(L, oldTop + 1);
            if (LuaDLL.lua_pcall(L, nArgs, 0, oldTop + 1) == 0)
            {
                LuaDLL.lua_remove(L, oldTop + 1);	//pop errorFunc
            }
            else
            {
                LuaDLL.lua_remove(L, oldTop + 1);	//pop errorFunc
            ...
            */
            ...
            string err = LuaDLL.Unity_Lua_Call(L, nArgs, LuaIndexes.LUA_REGISTRYINDEX, errorFuncRef);
            if (err != null)
                Common.HobaDebuger.LogError(err);
        }
        ...
        {
            int oldTop = LuaDLL.lua_gettop(L) - nArgs - 1;
	        if(LuaDLL.lua_pcall(L, nArgs,0, 0)==0)
                return;
            else
			{
                //ThrowExceptionFromError(oldTop);
        ...
        public bool PCall(int nArgs, int nResults)
        {
            ...
            LuaDLL.lua_rawgeti(L, LuaIndexes.LUA_REGISTRYINDEX, errorFuncRef);
            LuaDLL.lua_insert(L, oldTop + 1);
            if (LuaDLL.lua_pcall(L, nArgs, nResults, oldTop + 1) == 0)
            {
                LuaDLL.lua_remove(L, oldTop + 1);	//pop errorFunc
                return true;
            }
            ...
            */
            ...
            return LuaDLL.HOBA_Unity_Lua_PCall(L, nArgs, nResults, LuaIndexes.LUA_REGISTRYINDEX, errorFuncRef);
        }
