namespace LuaInterface
{
    ...
    {
        //fix 匿名委托 equals 判断出错bug, by topameng
        ...
        internal object[] popValues(IntPtr luaState, int oldTop)
        {
            ...
            {
                int len = Math.Max(newTop - oldTop, 0);
                object[] returnValues = new object[len];
                int k = 0;
                for (int i = oldTop + 1; i <= newTop; i++)
                {
                    returnValues[k] = GetObject(luaState, i);
                    ++k;
                }
        ...
        internal object[] popValues(IntPtr luaState, int oldTop, Type[] popTypes)
        {
            ...
            else
            {
                ...
                for (int i = oldTop + 1; i <= newTop; i++)
                {
                    returnValues[k] = getAsType(luaState, i, popTypes[iTypes]);
                    ++iTypes;
                    ++k;
                }
