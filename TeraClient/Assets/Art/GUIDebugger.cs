...
public class GUIDebugger : MonoBehaviour
{
    ...
    private void ExpansionGUIWindow(int windowId)
    {
        ...
        else if (_debugType == DebugType.Quality)
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("后处理开启关闭选择");
            Enable_Fog = GUILayout.Toggle(Enable_Fog, new GUIContent("Fog"));
            Enable_DepthOfField = GUILayout.Toggle(Enable_DepthOfField, new GUIContent("DOF"));
            Enable_HSV_Adjust = GUILayout.Toggle(Enable_HSV_Adjust, new GUIContent("HSV"));
            Enable_Bloom = GUILayout.Toggle(Enable_Bloom, new GUIContent("Bloom"));
            Enable_MotionBlur = GUILayout.Toggle(Enable_MotionBlur, new GUIContent("MotionBlur"));

            
            if (GUILayout.Button("调整完成，点击生效"))
            {
                SavePostPoseChain();
            }
            GUILayout.Label("：" + QualitySettings.names[QualitySettings.GetQualityLevel()] + value);
            GUILayout.EndHorizontal();
    ...
    bool Enable_Fog;
    bool Enable_DepthOfField;
    bool Enable_HSV_Adjust;
    bool Enable_Bloom;
    bool Enable_MotionBlur;
    string errorText = "";
    private void SavePostPoseChain()
    {
        ...
        var postComp = Main.Main3DCamera.GetComponent<PostProcessChain>();
        if (null == postComp)
        {
            errorText = "尚未加载后处理脚本";
            return;
        ...
        errorText = "已修改成功";
        postComp.DebugSet(Enable_Fog, Enable_DepthOfField, Enable_HSV_Adjust, Enable_Bloom, Enable_MotionBlur);
    }
