...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        m_bSupportRockchip = SystemInfo.deviceModel.Contains("rockchip");
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            if (m_bCheckFBO == false)
            {
                ...
                if (m_VideoTextureY != null)
                {
                    ...
                    {
                        UnityEngine.Object.Destroy(m_VideoTextureDummyY);
                ...
                if (m_VideoTextureU != null)
                {
                    ...
                    {
                        UnityEngine.Object.Destroy(m_VideoTextureDummyU);
                ...
                if (m_VideoTextureV != null)
                {
                    ...
                    {
                        UnityEngine.Object.Destroy(m_VideoTextureDummyV);
    ...
    public void DeleteVideoTexture()
    {
        ...
        if (m_VideoTextureY != null)
        {
            UnityEngine.Object.Destroy(m_VideoTextureY);
            m_VideoTextureY = null;
        }
        ...
        if (m_VideoTextureU != null)
        {
            UnityEngine.Object.Destroy(m_VideoTextureU);
            m_VideoTextureU = null;
        }
        ...
        if (m_VideoTextureV != null)
        {
            UnityEngine.Object.Destroy(m_VideoTextureV);
            m_VideoTextureV = null;
        }
    }
    ...
    {
        //DebugMethod("ResizeTexture " + m_iWidth + " " + m_iHeight);
    ...
    {
        //DebugMethod("ApplicationPause : " + bPause);
        if (bPause == true)
        {
    ...
    private bool m_bReadyPlay = false;
	...
	_videoTexture = new Texture2D (Call_GetVideoWidth (), Call_GetVideoHeight (), TextureFormat.RGB565, false);
	else
	_videoTexture = Texture2D.CreateExternalTexture((int)videoSize.x, (int)videoSize.y, TextureFormat.RGB565,
    ...
    private AVFrame* pConvertedFrame = null;
    private sbyte* pConvertedFrameBuffer = null;
    private SwsContext* pConvertContext = null;
    private AVCodecContext* pCodecContext = null;
    private AVCodecContext* pAudioCodecContext = null;
    private AVFrame* pDecodedFrame = null;
    private AVFrame* pDecodedAudioFrame = null;
    private AVFormatContext* pFormatContext = null;
    private AVPacket* pPacket = null;
    private AVStream* pStream = null;
    private AVStream* pStreamAudio = null;
    ...
    private bool bVideoFirstFrameReady = false;
    ...
    private unsafe bool Call_Load(string strFileName, int iSeek)
    {
        ...
        loader = new Thread(() =>
        {
            ...
            if (ffmpeg.avformat_open_input(&ppFomatContext, strFileName, null, null) != 0)
            {
                ...
                //DebugMethod("Could not open file");
            ...
            {
                m_CurrentState = MEDIAPLAYER_STATE.ERROR;
                //DebugMethod("Could not find stream info");
    ...
    private void LoadVideoPart2()
    {
        ...
        for (var i = 0; i < (pFormatContext)->nb_streams; i++)
        {
            if ((pFormatContext)->streams[i]->codec->codec_type == AVMediaType.AVMEDIA_TYPE_VIDEO)
            {
                if (bFindVideo == false)
                {
                    ...
                    //DebugMethod("Video" + iStreamIndex);
                }
            }
        ...
        {
            m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            //DebugMethod("Could not found video stream");
        ...
        {
            //m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            //DebugMethod("Could not found Audio stream" + bFindAudio);
        ...
        {
            m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            //DebugMethod("Could not initialize the conversion context");
        ...
        {
            m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            //DebugMethod("Could not open codec");
    ...
    private AVHWAccel* ff_find_hwaccel(AVCodecID codec_id, AVPixelFormat pix_fmt)
    {
    ...
    private VideoInterrupt action = null;
    ...
    private float fLastFrameTime = 0.0f;
    private float fCurrentSeekTime = 0.0f;
            ...
            if (pPacket != null)
            {
                if (pPacket->stream_index == iStreamIndex)
                {
                    ...
                    if (size < 0)
                    {
                        ...
                        //DebugMethod("Error while decoding frame");
                        return;
                    }
            ...
            do
            {
                ...
                if (ret < 0)
                {
                    ...
                    else
                    {
                        ...
                        //DebugMethod("Could not read frame");
                    }
                }
    ...
    private int iSoundBufferCount = 0;
    ...
    private bool m_bSetTextureFirst = false;
    ...
    public IEnumerator DownloadStreamingVideoAndLoad(string strURL)
    {
        ...
        //DebugMethod("DownloadStreamingVideo : " + strURL);
    ...
    private IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
    {
        ...
        {
            //DebugMethod("CopyStreamingAssetVideoAndLoad : " + strURL);
    ...
    #endregion UnityMainThread adapter
}