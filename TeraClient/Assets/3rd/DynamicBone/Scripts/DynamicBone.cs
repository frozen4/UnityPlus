...
public class DynamicBone : MonoBehaviour
{
    ...
    static Plane m_ProjectPlane = new Plane();
    ...
    void CheckDistance()
    {
        ...
        if (rt == null && Main.Main3DCamera != null)
            rt = Main.Main3DCamera.transform;
        if (rt != null)
        {
    ...
    void AppendParticles(Transform b, int parentIndex, float boneLength)
    {
        ...
        if (b != null)
        {
            ...
            if (b.childCount == 0 && (m_EndLength > 0 || !m_EndOffset.IsZero()))
    ...
    void UpdateParticles2()
    {
        for (int i = 1; i < m_Particles.Count; ++i)
        {
            ...
            if (m_FreezeAxis != FreezeAxis.None)
            {
                ...
                {
                    case FreezeAxis.X:
                        m_ProjectPlane.SetNormalAndPosition(p0.m_Transform.right, p0.m_Position);
                        break;
                    case FreezeAxis.Y:
                        m_ProjectPlane.SetNormalAndPosition(p0.m_Transform.up, p0.m_Position);
                        break;
                    case FreezeAxis.Z:
                        m_ProjectPlane.SetNormalAndPosition(p0.m_Transform.forward, p0.m_Position);
                        break;
                }
                p.m_Position -= m_ProjectPlane.normal * m_ProjectPlane.GetDistanceToPoint(p.m_Position);
            }
