...
namespace CinemaDirector
{
    ...
    public class Cutscene : MonoBehaviour, IOptimizable
    {
        ...
        List<float> _milestoneTimes = new List<float>();
        private List<float> getMilestones(float time)
        {
            // Create a list of ordered milestone times.

            _milestoneTimes.Clear();
            _milestoneTimes.Add(time);

            TrackGroup[] trackGroups = GetTrackGroups();
            for (int i = 0; i < trackGroups.Length; i++)
            {
                List<float> times = trackGroups[i].GetMilestones(RunningTime, time);
                for (int j = 0; j < times.Count; j++)
                {
                    if (!_milestoneTimes.Contains(times[j]))
                        _milestoneTimes.Add(times[j]);
                }
            }

            _milestoneTimes.Sort();
            if (time < RunningTime)
            {
                _milestoneTimes.Reverse();
            }

            return _milestoneTimes; 
        }
