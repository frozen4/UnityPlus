...
namespace CinemaDirector
{
    ...
    /// </summary>
    [TrackGroup("Track Group", TimelineTrackGenre.GlobalTrack)]
    public abstract class TrackGroup : MonoBehaviour, IOptimizable
    {
        ...
        protected List<TimelineTrack> trackCache;
        ...
        public virtual List<float> GetMilestones(float from, float to)
        {
            ...
            for (int i = 0; i < tracks.Count; i++)
            {
                ...
                {
                    if (!_times.Contains(trackTimes[j]))
                        _times.Add(trackTimes[j]);
                }
            }
            _times.Sort();
            return _times;
        ...
        public virtual List<TimelineTrack> GetTracks()
        {
