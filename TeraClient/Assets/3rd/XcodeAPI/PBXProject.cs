...
#endif
{
    ...
    public class PBXProject
    {
        ...
        public void AddDynamicFrameworksToProject(string targetGuid, string[] frameworksPathInProject)
        {   
            ...
            if (m_Data.copyFiles.HasEntry(embedFrameworksSection.guid))
                embedFrameworksSection = m_Data.copyFiles[embedFrameworksSection.guid];
            else
                m_Data.copyFiles.AddEntry(embedFrameworksSection);

            foreach (var framework in frameworksPathInProject)
            {
                var fileGuid = FindFileGuidByProjectPath(framework);
                if (fileGuid == null)
                {
                    UnityEngine.Debug.LogError("Framework not found: " + framework);
                    continue;
                }
