...
public class BMFontTool : EditorWindow
{
    ...
    private void CreateFontAndMaterial(string localPath)
    {
        string directoryName = Path.GetDirectoryName(localPath);
        string fileName = Path.GetFileNameWithoutExtension(localPath);
        string filePath;

        fontMaterial = AssetDatabase.LoadAssetAtPath<Material>(directoryName + "" + fileName + ".mat");
        if (fontMaterial == null)
        {
            filePath = directoryName + "/" + fileName + ".mat";
            fontMaterial = new Material(Shader.Find("UI/Default"));
            fontMaterial.mainTexture = fontTexture;
            fontSavePath = filePath;
        ...
        targetFont = AssetDatabase.LoadAssetAtPath<Font>(directoryName + "" + fileName + ".fontsettings");
        if (targetFont == null)
        {
            filePath = directoryName + "/" + fileName + ".fontsettings";
            targetFont = new Font(fileName);
    ...
    [MenuItem("Hoba Tools/Import BMFont")]
    // Use this for initialization
    static public void OpenBMFontMaker()
    {
        EditorWindow.GetWindow<BMFontTool>(false, "BMFont Maker", true).Show();
    }
}
