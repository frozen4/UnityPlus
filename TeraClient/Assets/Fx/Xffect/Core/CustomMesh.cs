       ...
       // EffectNode Owner;
        ...
        public void Transform()
        {
            ...
            if (Node.Owner.DirType != DIRECTION_TYPE.Planar)//NOTE, PLANAR DIR JUST IGNORE IT..
            {
                ...
                rotation = rot * MyRotation;
            }

            if (Node.Owner.AlwaysSyncRotation)
            {
                Quaternion prot = Node.Owner.transform.rotation;
            ...
            LocalMat.SetTRS(Vector3.zero, rotation, scale);
            //WorldMat.SetTRS(MyPosition, Quaternion.identity, Vector3.one);
            WorldMat = Matrix4x4.Translate(MyPosition);
