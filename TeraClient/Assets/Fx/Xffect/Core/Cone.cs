    ...
    public class Cone : RenderObject
    {
        ...
        public void UpdateUV()
        {
            ...
            for (int i = 0; i < NumSegment + 1; i++)
            {
                ...
                float x = pool.UVs[index + i].x + i * uvSeg;
                float y = LowerLeftUV.y;
                pool.UVs[index + i] = new Vector2(x, y);
            }
            for (int i = NumSegment + 1; i < NumVerts; i++)
            {
                ...
                float x = pool.UVs[index + i].x + (i - NumSegment - 1) * uvSeg;
                float y = LowerLeftUV.y + UVDimensions.y;
                pool.UVs[index + i] = new Vector2(x, y);
            }
