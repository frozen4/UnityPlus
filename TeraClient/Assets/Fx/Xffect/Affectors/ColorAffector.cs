...
namespace Xft{
    ...
    public class ColorAffector : Affector
    {
        ...
        public override void Reset()
        {
			...
			{
                Common.HobaDebuger.LogWarningFormat("invalid color gradual time, loop node can't be gradient by 'gradual time': {0} {1}", mOwner.ColorGradualTimeLength, mOwner.name);
			}
        }
