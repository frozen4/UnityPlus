...
public static class ToLuaExport 
{
    ...
    public static bool wrapAllMethods = true;
    public static List<string> methodsFilter = null;
    public static bool wrapAllFields = true;
    public static List<string> fieldsFilter = null;
    ...
    public static void Generate(params string[] param)
    {
        ...
        if (type.IsEnum)
        {
            ...
            GenEnumTranslator();
            sb.AppendLine("}\r\n");
            SaveFile(Application.dataPath + WrapFileSavedPath + wrapClassName + "Wrap.cs");
            return;
        }
        ...
        else
        {
            ...
            for (int i = list.Count - 1; i >= 0; --i)
            {
                ...
                if (!wrapAllMethods)
                {
                    if (methodsFilter == null || !methodsFilter.Contains(list[i].Name))
                        list.RemoveAt(i);
                }
            }
        }        
        ...
        GenRegFunc();
        //GenConstruct();  // 除GameObject外，lua中不需要New
    ...
    static void GenLuaFields()
    {
        ...
        for (int i = fieldList.Count - 1; i >= 0; i--)
        {
            ...
            if(!wrapAllFields)
            {
                if(fieldsFilter == null || !fieldsFilter.Contains(fieldList[i].Name))
                    fieldList.RemoveAt(i);
            }
        }
        ...
        {            
            ...
            if (!wrapAllFields)
            {
                if (fieldsFilter == null || !fieldsFilter.Contains(piList[i].Name))
                    piList.RemoveAt(i);
            }
        }
...
//         }
    ...
    {        
            ...
            t == typeof(LuaStringBuffer) || typeof(UnityEngine.TrackedReference).IsAssignableFrom(t) || typeof(IEnumerator).IsAssignableFrom(t))
        {
            ...
            t == typeof(Ray) || t == typeof(Touch) || t == typeof(Bounds) || t == typeof(Rect))
        {
            return "Push";
        }
    ...
    static void GenIndexFunc()
    {
        for(int i = 0; i < fields.Length; i++)
        {
            ...
            {
                if (fields[i].FieldType.FullName.Contains("System.Collections.Generic.List"))
                {
                    sb.AppendFormat("\t\tobject o = LuaScriptMgr.GetLuaObject(L, 1);\r\n");
                    sb.AppendFormat("\t\t{0} obj = ({0})o;\r\n", className);
                    sb.AppendLine();
                    sb.AppendLine("\t\tif (obj != null)\n\t\t{");
                    sb.AppendLine("\t\t\tLuaDLL.lua_newtable(L); ");
                    sb.AppendFormat("\t\t\tfor (int i = 0; i < obj.{0}.Count; i++)\r\n", fields[i].Name);
                    sb.AppendLine("\t\t\t{");
                    sb.AppendLine("\t\t\t\tLuaScriptMgr.Push(L, i + 1);");
                    sb.AppendFormat("\t\t\t\tLuaScriptMgr.PushObject(L, obj.{0}[i]);\r\n", fields[i].Name);
                    sb.AppendLine("\t\t\t\tLuaDLL.lua_settable(L, -3);");
                    sb.AppendLine("\t\t\t}");
                    sb.AppendLine("\t\t}");
                    sb.AppendLine("\t\telse");
                    sb.AppendLine("\t\t{");
                ...
                else if (fields[i].FieldType.FullName.Contains("System.Collections.Generic.Dictionary"))
                {
                    sb.AppendFormat("\t\tobject o = LuaScriptMgr.GetLuaObject(L, 1);\r\n");
                    sb.AppendFormat("\t\t{0} obj = ({0})o;\r\n", className);
                    sb.AppendLine();
                    sb.AppendLine("\t\tif (obj != null)\n\t\t{");
                    sb.AppendLine("\t\t\tLuaDLL.lua_newtable(L); ");
                    string full_name = fields[i].FieldType.ToString();
                    full_name = full_name.Replace("`2[", "<");
                    full_name = full_name.Replace(']', '>');
                    sb.AppendFormat("\t\t\t{1}.Enumerator e = obj.{0}.GetEnumerator();\r\n", fields[i].Name, full_name);
                    sb.AppendLine("\t\t\twhile (e.MoveNext())");
                    sb.AppendLine("\t\t\t{");                   
                    sb.AppendLine("\t\t\t\tvar key = e.Current.Key;");
                    sb.AppendLine("\t\t\t\tvar value = e.Current.Value;");
                    sb.AppendLine("\t\t\t\tLuaScriptMgr.Push(L, key);");
                    sb.AppendLine("\t\t\t\tLuaScriptMgr.PushObject(L, value);");
                    sb.AppendLine("\t\t\t\tLuaDLL.lua_settable(L, -3);");
                    sb.AppendLine("\t\t\t}");
                    sb.AppendLine("\t\t}");
                    sb.AppendLine("\t\telse");
        ...
        for (int i = 0; i < props.Length; i++)
        {
            ...
            {
                if (props[i].PropertyType.ToString().Contains("System.Collections.Generic.List"))
                {
                    sb.AppendFormat("\t\tobject o = LuaScriptMgr.GetLuaObject(L, 1);\r\n");
                    sb.AppendFormat("\t\t{0} obj = ({0})o;\r\n", className);
                    sb.AppendLine();
                ...
                else if (props[i].PropertyType.FullName.Contains("System.Collections.Generic.Dictionary"))
                {
                    Debug.LogError(props[i].PropertyType.FullName);
                    ...
                    sb.AppendLine("\t\tif (obj != null)\n\t\t{");
                    sb.AppendLine("\t\t\tLuaDLL.lua_newtable(L); ");
                    string full_name = props[i].PropertyType.ToString();
                    full_name = full_name.Replace("`2[","<");
                    full_name = full_name.Replace(']', '>');
                    sb.AppendFormat("\t\t\t{1}.Enumerator e = obj.{0}.GetEnumerator();\r\n", props[i].Name, full_name);
                    sb.AppendLine("\t\t\twhile (e.MoveNext())");
                    sb.AppendLine("\t\t\t{");
                    sb.AppendLine("\t\t\t\tvar key = e.Current.Key;");
                    sb.AppendLine("\t\t\t\tvar value = e.Current.Value;");
