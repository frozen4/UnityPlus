...
// SH/ambient and vertex lights
...
#endif
#if defined(_TERRAIN_NORMAL_MAP) || defined(RAIN_SURFACE_ON)
	...
	// The FirstPassShader is relatively expensive on the GPU, and is not necessarily supported by older graphics cards.
	// so using the Dependency "BaseMapShader" shader
	//Dependency "BaseMapShader" = "Hidden/TerrainEngine/Splatmap/Standard-Base"

	Dependency "AddPassShader" = "Diffuse"
	...
	//Q: why change "Mobile/Diffuse" to "Diffuse" ?
	//A: In Class DetailRenderer's constructor function:
	/*
	const char* shaders[] = {
	...
	for (int i = 0; i < kDetailRenderModeCount; i++)
	{
		...
		m_Materials[i] = Material::CreateMaterial (*shader, Object::kHideAndDontSave);  // if shader == null, crashed!
		m_Materials[i]->SetFloat (kSLPropCutoff, .5f * .75f);
	}
	*/
	// If Details0 - Details2 can not be found, "Diffuse" is the default option; 
	// If the default option was missing, Crashed!!!
